...
I generally made three different prediction sover the course of ~1.5 months: ① based on weighted high-quality polls only (I downloaded those from 538); ② adjusted by the full average polling bias from the 2016 and 2020 elections; and ③ adjusted by just **half** the 2016/20 polling bias.
...
Beginning of November -- based on very reasonable-sounding arguments -- I also started looking into whether the polling errors from 2012 and 2022 might be relevant to consider. Unfortunately, those arguments turned out to be completely, utterly wrong, so we'll exclude predictions ④ and ⑤ in this post-mortem analysis.
